...
class SpawnpointDetectionData(BaseModel):
    ...
    def classify_1x60(cls, query, sp):
        ...
        # times holds pairs of earliest and latest times that a given pokemon was present
        times = []
        for encounter_id, scan_times in sights_by_encounter.iteritems():
            ...
            for scan_time in scan_times:
                ...
                # scan_times array is ordered, so if `st` is smaller, then it means we passed the hour.
                # for all the following times, add 3600 seconds so we maintain an order we can use to determine the
        ...
        # now lets find out the earliest and the latest time that this spawnpoint has had visible pokemons
